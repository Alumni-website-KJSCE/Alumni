...
import { FaMapLocation, FaBuilding, FaMoneyBillWave, FaClock, FaLocationDot, FaCalendarDays, FaPlus, FaTrash } from "react-icons/fa6";
...
const Career = () => {
  ...
  const [isUserApprovedForPosting, setIsUserApprovedForPosting] = useState(false);
  ...
  const filterOpportunities = (opportunities, filters) => {
    return opportunities.filter(opportunity => {
      ...
      // Experience filter
      if (filters.experience && opportunity.experience !== filters.experience) {
        return false;
      }
  ...
  const handleJobUpdated = (updatedJob) => {
    ...
    setAllOpportunities(prev => prev.map(opportunity => 
  ...
  useEffect(() => {
    if (searchParams.get('posted') === 'true') {
      setSuccessMessage('Job posted successfully! It will be visible after admin approval.');
      setTimeout(() => {
        setSuccessMessage('');
        // Clean up URL
    ...
    const fetchJobsAndInternships = async () => {
      try {
        ...
        if (user && profileLoaded && !isUserApprovedForPosting) {
          setError('Your account is pending approval. You can view jobs but cannot post new ones.');
        }
      } catch (fetchError) {
  ...
  const handleDeleteJob = async (jobId, jobType) => {
    ...
    try {
      ...
      setAllOpportunities(prev => prev.filter(opportunity => opportunity._id !== jobId));
      setFilteredOpportunities(prev => prev.filter(opportunity => opportunity._id !== jobId));
  ...
  if (error && error !== 'Your account is pending approval. You can view jobs but cannot post new ones.') {
    console.log('Top-level error:', error);
    return <div className="career-container"><p style={{color:'red',textAlign:'center'}}>{error}</p></div>;
  }
  ...
  console.log('Render Career: isUserApprovedForPosting:', isUserApprovedForPosting);
  console.log('Render Career: showJobForm:', showJobForm);
  console.log('Render Career: token in localStorage:', localStorage.getItem('token') ? 'present' : 'missing');
    ...
    <div className="page-container">
      ...
      <section className="section" aria-label="careers section">
        ...
        <div className="career-hero">
          ...
          <p>Find your next career opportunity or share job openings with the alumni community</p>
        </div>
        ...
        <div className="tabs-container">
          ...
          {isLoggedIn && isUserApprovedForPosting && !showJobForm && (
            <div className="post-job-button-container">
              ...
              >
                <FaPlus className="post-icon" aria-hidden="true" /> Post a Job/Internship
              </button>
            </div>
          )}
            ...
            <div className="opportunities-container">
              <div className="section-header">
                ...
                {filteredOpportunities && filteredOpportunities.length > 0 && (
                  <button className="view-all-button" aria-label="View all opportunities">
              ...
              {isLoading ? (
                ...
                <div className="job-cards" role="list">
                  {filteredOpportunities && filteredOpportunities.map((opportunity, index) => {
                    console.log('=== OPPORTUNITY OWNERSHIP DEBUG ===');
                    console.log('Opportunity data:', opportunity);
                    console.log('Opportunity postedBy:', opportunity.postedBy);
                    console.log('Opportunity postedBy type:', typeof opportunity.postedBy);
                    console.log('Current user from AuthContext:', user);
                    console.log('Current userId from AuthContext:', userId);
                    console.log('Current userId type:', typeof userId);
                    
                    if (opportunity.postedBy && typeof opportunity.postedBy === 'object' && opportunity.postedBy._id) {
                      console.log('postedBy._id:', opportunity.postedBy._id);
                      console.log('postedBy._id type:', typeof opportunity.postedBy._id);
                      console.log('postedBy._id toString():', opportunity.postedBy._id.toString());
                    }
                    
                    const isOwner = opportunity.postedBy && userId &&
                      ((typeof opportunity.postedBy === 'object' && opportunity.postedBy._id && opportunity.postedBy._id.toString() === userId.toString()) ||
                       (typeof opportunity.postedBy === 'string' && opportunity.postedBy === userId.toString()));
                    console.log('Is owner result:', isOwner);
                    console.log('=== END DEBUG ===');
                    
                    return (
                      <div key={opportunity._id || index} className="job-card">
                        {/* Job Type Badge */}
                        <div className="job-type-badge">
                          {opportunity.jobType === 'job' ? 'Job' : 'Internship'}
                        </div>
                        ...
                        {isOwner && (
                          <div className="job-management">
                            ...
                            <button
                              ...
                              onClick={(e) => {
                                e.stopPropagation();
                                handleDeleteJob(opportunity._id, opportunity.jobType);
                              }}
                        ...
                        <div className="job-actions">
                          ...
                          {isOwner && (
                            <div className="posted-by-badge">Created by you</div>
                          )}
                        </div>
                      </div>
                    );
          ...
          >
            <div className="alumni-map-container">
              <h2 className="section-title">Top Cities Where Our Alumni Work</h2>
